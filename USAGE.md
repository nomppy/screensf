...
| `REPERTORY_ONLY` | no | `1` to never auto-include a film newer than two years; every first-run title waits for your call in review |
...
- **Edit details**: opens a form for title, year, director, runtime, genre, description and the card note (the red italic line on the site card: Q&A, series name, print info). Anything you change overrides TMDB on the site; clear a field to hide it. *Reset to TMDB* drops the edits.
...
You are never asked about the same title twice. A second chip row filters by the classifier's reason (No TMDB match, Uncertain match, Now playing, New release, and so on) with counts for the current tab and venue. The header chips filter by status (Pending, Included, Title only, Auto-included, Excluded, Auto-excluded, Edited), by venue, and by classifier reason. Every row is multi-select: click several chips to combine them, and the All or Any chip clears that row. Each chip shows a count. Classifier calls carry a grey auto badge and the rule that fired; press Include or Exclude on one to override it. Open a decided card from any of them and use Undo, or change the selection and press Save changes.
...
Titles the classifier is unsure about never publish on their own; they wait for you. Confident matches publish, confident junk is dropped. To be stricter or looser, set `REPERTORY_ONLY`, `REPERTORY_POPULARITY` (what gets auto-included), `FIRST_RUN_SHOWTIMES` and `BLOCKBUSTER_POPULARITY` (what gets auto-excluded) in `.env` locally and as repository **Variables** on GitHub.
