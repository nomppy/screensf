...
You are never asked about the same title twice. A second chip row filters by the classifier's reason (No TMDB match, Uncertain match, Now playing, New release, and so on) with counts for the current tab and venue. The header tabs filter by status: Pending, Included (everything on the site, whether matched to TMDB, title only, or included automatically by the classifier), Excluded, Edited (a hand-edited card or a custom artwork pick), and All, each with a count. Classifier calls carry a grey auto badge and the rule that fired; press Include or Exclude on one to override it. Open a decided card from any of them and use Undo, or change the selection and press Save changes.
